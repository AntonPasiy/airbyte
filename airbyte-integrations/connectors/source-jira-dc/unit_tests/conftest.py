...
def mock_non_deleted_projects_responses(config, projects_response):
    responses.add(
        responses.GET,
        f"https://{config['domain']}/rest/api/2/project?maxResults=50&expand=description%2Clead&status=live&status=archived",
        json=projects_response,
    )
...
def find_stream(stream_name, config):
    for stream in SourceJiraDc().streams(config=config):
